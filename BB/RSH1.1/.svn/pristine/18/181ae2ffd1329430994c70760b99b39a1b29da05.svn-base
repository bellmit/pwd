...
/**
 * 首页-商会通-机构-办事指南-查看更多列表
 *
 * @author shenbh
 * @time 2018/1/9 11:19
 */
public class HomeBSZNCKListActivity extends MyAppCompatActivity implements PullRefreshPushAddListView.IXListViewListener {
                ...
                try {
                    ...
                    switch (msg.what) {
                        case Constants.WHAT_REFRESH:
                            ((HomeBSZNCKListActivity) t).pageIndex = Constants.PAGE_START_INDEX;
                            ((HomeBSZNCKListActivity) t).adapter.notifyDataSetChanged();
                            ((HomeBSZNCKListActivity) t).onLoad();
                            break;
                        case Constants.WHAT_LOADMORE:
                            ++((HomeBSZNCKListActivity) t).pageIndex;
                            ((HomeBSZNCKListActivity) t).adapter.notifyDataSetChanged();
                            ((HomeBSZNCKListActivity) t).onLoad();
                            break;
                        case Constants.WHAT_GETDATA:
                            if (null != resInfo) {
                                if (((HomeBSZNCKListActivity) t).pageIndex > Constants.PAGE_START_INDEX && resInfo.getClass(HomeZixunBean.class).getData().size() == 0) {
                                    ToastUtils.showOneToast(t.getApplicationContext(), "没有更多");
                                    return;
                                }
                                ((HomeBSZNCKListActivity) t).list.addAll(resInfo.getClass(HomeZixunBean.class).getData());
                            }
                            ((HomeBSZNCKListActivity) t).plv.setAdapter(((HomeBSZNCKListActivity) t).adapter = new CommonAdapter<HomeZixunBean.DataBean>(
                                    t.getApplicationContext(), ((HomeBSZNCKListActivity) t).list, R.layout.item_rsnewslist
                            ) {
                                @Override
                            ...
                            ((HomeBSZNCKListActivity) t).plv.setOnItemClickListener(new AdapterView.OnItemClickListener() {
                                @Override
                                public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
                                    WeakReference<ShowWebBean> wf = new WeakReference<ShowWebBean>(new ShowWebBean());
                                    wf.get().setUrl(((HomeBSZNCKListActivity) t).list.get(position - 1).getDetail());
                                    wf.get().setTitle("详情");
...
//采用okhttp3来进行网络请求
        final String url = UrlManager.URL_GETPOSTBYCOLUMN;
        OkHttpClient mOkHttpClient = new OkHttpClient();

        FormBody.Builder builder = new FormBody.Builder();
        builder.add("module_id", homeDetailListBean.getModuleId() + "");
    ...
    public void onViewClicked() {
        HomeBSZNCKListActivity.this.finish();
    }
