...
public class PicassoUtils {
    ...
    public void loadImageByUri(Context context, Uri uri, ImageView imageView, String bitmapShowType, float roundRadius) {
        ...
        if (bitmapShowType.equals(PICASSO_BITMAP_SHOW_CIRCLE_TYPE)) {
            ...
            Picasso.with(context).load(uri).placeholder(R.drawable.person_defaultheadportrait).error(R.drawable.pic1).transform(new RoundTransform(roundRadius)).into(imageView);
        } else {
...
//            Picasso.with(context).load(path).resize(targetWidth, targetHeight).centerCrop().transform(new RoundTransform(roundRadius)).into(imageView);
            ...
            if (targetWidth != -1 && targetHeight != -1) {//对图片进行裁剪，会导致图片失真
                Picasso.with(context).load(path).resize(targetWidth, targetHeight).centerCrop().error(R.drawable.ic_launcher).transform(new PicassoRoundTransform(roundRadius)).into(imageView);
            } else {//直接使图片自适应控件大小，不失真
                Picasso.with(context).load(path).error(R.drawable.ic_launcher).fit().transform(new PicassoRoundTransform(roundRadius)).into(imageView);
            }
        } else {
