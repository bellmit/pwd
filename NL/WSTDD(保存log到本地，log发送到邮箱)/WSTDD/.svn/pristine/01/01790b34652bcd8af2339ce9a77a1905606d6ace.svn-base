 ...
 *
 */
public class HttpMultipartPostPersonCenter extends AsyncTask<String, Integer, String> {
...
//	private ProgressDialog pd;
	...
	public HttpMultipartPostPersonCenter(Context context, List<String> filePathList) {
			...
			progressDialog.setOnKeyListener(new OnKeyListener() {
				...
				public boolean onKey(DialogInterface dialog, int keyCode, KeyEvent event) {
					...
					if(progressDialog!=null&&progressDialog.isShowing()){
						...
						progressDialog = null;
						((RegistFinishActivity)context).setLocalBitmap();
						}
					return false;
				}
			});
...
//		WBResponse response= MessageUtil.JsonStrToWBResponse(result,cls);
		((MinePersonalCenterActivity) context).handleHeadImg(result);//传完之后的操作
		//上传的结果显示在这里
//		pd.dismiss();
//		pd = null;
		if(progressDialog!=null){
