 ...
 * 
 */
public class SxRegistrationSubmitAdapter extends BaseAdapter {
	private LayoutInflater mInflater;
	
	private Context context;
	private SxRegistrationSubmitChildAdapter adapter;
	private List<SxRegistrationSubmitAdapterData> registrationData;// 数据
	private boolean showRl1 = true;// 第一个rl布局需要显示出来 姓名 手机号 删除
	private boolean showRl2 = true;// 第二个rl布局需要显示出来 编辑信息 完成

	public SxRegistrationSubmitAdapter(Context context, List<SxRegistrationSubmitAdapterData> registrationData) {
	...
	@Override
	public SxRegistrationSubmitAdapterData getItem(int position) {
	...
	public View getView(final int position, View convertView, ViewGroup parent) {
		...
		if (convertView == null) {
			...
			// 根据自定义的Item布局加载布局
			convertView = mInflater.inflate(R.layout.registration_submit_registration_listview_item, parent, false);// 这里有区别？？？
			...
			holder.listView = (SxRegistrationSubmitListViews) convertView.findViewById(R.id.registration_sx_listview);
		...
		final SxRegistrationSubmitAdapterData data = getItem(position);
		holder.delete.setText("删除");
		holder.edit.setText("完成");
		adapter = new SxRegistrationSubmitChildAdapter(context, registrationData, position);
...
//					adapter.notifyDataSetChanged();
	...
	class ViewHolder {
		...
		public SxRegistrationSubmitListViews listView;// 动态添加
		// 需要进行显示隐藏的
	...
	public void setRegistrationData(List<SxRegistrationSubmitAdapterData> registrationData) {
		this.registrationData = registrationData;
	}
