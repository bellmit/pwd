	...
	public void refresh() {
		...
		if (tdTddActivitySignVo != null && StringUtil.isNotEmpty(tdTddActivitySignVo.getActivityId()) ) {
			...
			if (StringUtil.isNotEmpty(tdTddActivitySignVo.getSignHeadimgurl())) {
				ImageDownLoad.getDownLoadCircleImg(tdTddActivitySignVo.getSignHeadimgurl(),
						mHeadimg_iv);//头像
			}else {
			...
			tuanda_iv.setImageDrawable(getResources().getDrawable(R.drawable.test_applylist_td));
			// mPhone_iv ;//手机
...
//				mAllchose_checkbox.setChecked(false);
		...
		// passTYAdapter.setOnItemClickListener(new
		// com.newland.wstdd.mine.applyList.PassTYAdapter.onItemClickListener()
		// {
		//
		// @Override
		// public void onItemClick(View v, int position) {
