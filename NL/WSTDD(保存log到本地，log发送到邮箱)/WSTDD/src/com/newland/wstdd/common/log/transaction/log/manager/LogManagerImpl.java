...
import com.newland.wstdd.common.log.transaction.log.task.LogTask;
...
public class LogManagerImpl implements ILogManager, UncaughtExceptionHandler {
	...
	private void checkExecutor() {
		if (mExecutorService == null || mExecutorService.isShutdown()) {
			...
			mExecutorService = Executors.newFixedThreadPool(mAvailableProcessors, new ThreadFactory() {
				@Override
	...
	public void uncaughtException(Thread thread, Throwable ex) {
		try {
			...
			if (LogUtils.CRASH_UPLOAD_2_NETWORK) {
				...
				submitTask(new LogTask(mContext, TAG, msg, LogUtils.LOG_TYPE_2_NETWORK));
			}
		} catch (Exception e) {
		}
