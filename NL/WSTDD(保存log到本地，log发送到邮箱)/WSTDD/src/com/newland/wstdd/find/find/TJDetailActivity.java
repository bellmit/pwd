 ...
 * 
 */
public class TJDetailActivity extends BaseFragmentActivity implements OnClickListener, OnPostListenerInterface {
	private static final String TAG = "TJDetailActivity";//收集异常日志tag
	// // 详情
	...
	private TJCollectHandle collecthandler = new TJCollectHandle(this);
	private String whichQuest;// 是什么请求 ：收藏/点赞/团大
	// 点赞
	private LikeRes likeRes;
	private String likeType = "1";// 操作（0 点赞 1 取消）
	private TJLikeHandle likehandler = new TJLikeHandle(this);
	...
	public void onClick(View v) {
		...
		case R.id.activity_find_collect:// 收藏
			// UiHelper.ShowOneToast(TJDetailActivity.this, "收藏");
			whichQuest = "collect";
			refresh();
			break;
		case R.id.activity_find_like:// 点赞
			// UiHelper.ShowOneToast(TJDetailActivity.this, "点赞");
			whichQuest = "like";
			refresh();
			break;
		case R.id.activity_find_register_btn:// 我要报名
			Intent intent = new Intent(TJDetailActivity.this, RegistrationSubmitActivity.class);
		...
		try {
			new Thread() {
				public void run() {
					...
					if ("collect".equals(whichQuest)) {// 收藏请求
						...
						message.what = TJCollectHandle.COLLECT;// 设置死
						// 访问服务器成功 1 否则访问服务器失败
						if (ret.getCode() == 1) {
						...
						Message message = new Message();
						message.what = TJLikeHandle.LIKE;// 设置死
						// 访问服务器成功 1 否则访问服务器失败
						if (ret.getCode() == 1) {
