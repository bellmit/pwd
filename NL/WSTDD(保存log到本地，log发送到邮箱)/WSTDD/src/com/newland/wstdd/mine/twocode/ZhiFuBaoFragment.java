...
import com.newland.wstdd.mine.twocode.handle.TwoCodeZhiFuBaoHandle;
...
public class ZhiFuBaoFragment extends BaseFragment implements OnPostListenerInterface {
	...
	/* 头像名称 */
	private static final String PHOTO_FILE_NAME = "zhifubao.png";
	private File tempFile;
	private List<String> filePathList;// 保存本地图片的地址
	private Uri uriImg;// 获取到了图片的Uri
	HttpMultipartPostZhiFuBaoFragment post;//
	...
	private TwoCodeZhiFuBaoHandle handler = new TwoCodeZhiFuBaoHandle(this);

	@SuppressLint("ValidFragment")
	public ZhiFuBaoFragment(Context context, Bitmap bitmap) {
	...
	protected View createAndInitView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
		View view = inflater.inflate(R.layout.fragment_zhifubao, container, false);
		initView(view);// 初始化控件
		return view;
	}

	private void initView(View view) {
		addTwoCode = (ImageView) view.findViewById(R.id.fragment_zhifubao_addimg);
		addTwoCode.setOnClickListener(new MyClick());
		changeTextView = (TextView) view.findViewById(R.id.fragment_zhifubao_text_change);
		changeTextView.setOnClickListener(new MyClick());
		modifyTextView = (TextView) view.findViewById(R.id.fragment_zhifubao_modify_tv);
	...
	public class MyClick implements OnClickListener {
		...
		public void onClick(View view) {
			...
			switch (view.getId()) {
			// 添加添加二维码的图片
			case R.id.fragment_zhifubao_addimg:
				showDownLoadDialog();
				break;
			case R.id.fragment_zhifubao_modify_tv:
				showDownLoadDialog();
				break;
			default:
				break;
	...
	private void showDownLoadDialog() {
		AlertDialog dialog = new AlertDialog.Builder(getActivity()).setTitle("温馨提示").setMessage("选择获取头像方式").setPositiveButton("本地相册", new android.content.DialogInterface.OnClickListener() {
	...
	public void upload() {
		...
		post = new HttpMultipartPostZhiFuBaoFragment(getActivity(), this, filePathList);
		post.execute();
	}
		...
		try {
			new Thread() {
				public void run() {
					...
					reqInfo.setAliPay(headImgUrlString);
					reqInfo.setQqPay(((TwoDimensionCodeActivity) context).getWeixinUrl());
					BaseMessageMgr mgr = new HandleNetMessageMgr();
					RetMsg<TwoCodePayRes> ret = mgr.getTwoCodePayInfo(reqInfo, AppContext.getAppContext().getUserId());// 泛型类，
					Message message = new Message();
					message.what = TwoCodeZhiFuBaoHandle.TWO_CODE_ZHIFUBAO;// 设置死
					// 访问服务器成功 1 否则访问服务器失败
					if (ret.getCode() == 1) {
	...
	public void OnHandleResultListener(Object obj, int responseId) {
		try {
			switch (responseId) {
			case TwoCodeZhiFuBaoHandle.TWO_CODE_ZHIFUBAO:
			
				twoCodePayRes = (TwoCodePayRes) obj;
				if (twoCodePayRes != null) {
					headImgUrlString = twoCodePayRes.getAliPay();
					if (headImgUrlString != null && !"".equals(headImgUrlString)) {
						// 显示布局控件
						modifyTextView.setVisibility(View.VISIBLE);
						changeTextView.setText("我的支付宝收款码");
						changeTextView.setBackground(getActivity().getResources().getDrawable(R.drawable.textview_zhifubaotwocode_radius_shap));
						// 根据url下载图片
						// 说明有图片了~从服务器上下载图片
						...
						ImageLoader.getInstance().displayImage(UrlManager.uploadToUrlServer + headImgUrlString, addTwoCode, new ImageLoadingListener() {
							...
							public void onLoadingComplete(String arg0, View arg1, Bitmap arg2) {
								// TODO Auto-generated method stub
								((TwoDimensionCodeActivity) context).setZhifubaoBitmap(arg2);
								((TwoDimensionCodeActivity) context).setZhifubaoUrl(headImgUrlString);
								localBitmap = arg2;
