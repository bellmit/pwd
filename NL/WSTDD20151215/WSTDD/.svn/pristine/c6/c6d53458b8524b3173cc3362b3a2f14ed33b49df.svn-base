...
public class SelectedImagesAdapter extends BaseAdapter {
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		if(arrayList.get(position).getImagePath()==null||"".equals(arrayList.get(position).getImagePath())){
			holder.imageView.setImageDrawable(context.getResources().getDrawable(R.drawable.empty_photo));
		}else {
			...
			if(file.exists()){
				...
				imageLoader.displayImage(UrlManager.uploadToUrlServer+arrayList.get(position).getImagePath(), holder.imageView,
						options);
			}
