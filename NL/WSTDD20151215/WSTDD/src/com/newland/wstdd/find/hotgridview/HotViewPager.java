 ...
 */
@SuppressLint("NewApi")
public class HotViewPager extends Fragment implements OnPageChangeListener {

	private List<ImageView> imageViews = new ArrayList<ImageView>();
	private ImageView[] indicators;
	private FrameLayout viewPagerFragmentLayout;
	private LinearLayout indicatorLayout; // 指示器
	private HotBaseViewPager viewPager;
	private HotBaseViewPager parentViewPager;
	private ViewPagerAdapter adapter;
	private HotViewPagerHandler handler;
	...
	private HotCycleListener mHotCycleListener;
	private List<HotListInfo> infos;

	@Override
	public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
		View view = LayoutInflater.from(getActivity()).inflate(R.layout.view_cycle_viewpager_contet_second, null);

		viewPager = (HotBaseViewPager) view.findViewById(R.id.viewPager);
		...
		handler = new HotViewPagerHandler(getActivity()) {
...
//					}
	...
	public void setData(List<ImageView> views, List<HotListInfo> list, HotCycleListener listener) {
	 ...
	 *            默认显示位置
	 */
	public void setData(List<ImageView> views, List<HotListInfo> list, HotCycleListener listener, int showPosition) {
		mHotCycleListener = listener;
		infos = list;
...
//		}
		...
		for (int i = 0; i < indicators.length; i++) {
			View view = LayoutInflater.from(getActivity()).inflate(R.layout.view_cycle_viewpager_indicator_second, null);
	...
	public void setIndicatorCenter() {
		...
		indicatorLayout.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				...
				UiHelper.HotListListViewActivity(getActivity());
	 ...
	 * @return viewPager
	 */
	public HotBaseViewPager getViewPager() {
		return viewPager;
	}
	...
	private class ViewPagerAdapter extends PagerAdapter {
		...
		public View instantiateItem(ViewGroup container, final int position) {
			...
			if (mHotCycleListener != null) {
				v.setOnClickListener(new OnClickListener() {

					@Override
					public void onClick(View v) {
						try {
							mHotCycleListener.onImageClick(infos.get(currentPosition - 1), currentPosition, v);
						} catch (Exception e) {
...
//		}
	 ...
	 */
	public void disableParentViewPagerTouchEvent(HotBaseViewPager parentViewPager) {
		if (parentViewPager != null)
	 ...
	 * @author minking
	 */
	public static interface HotCycleListener {
		 ...
		 * @param imageView
		 */
		public void onImageClick(HotListInfo info, int postion, View imageView);
	}
}