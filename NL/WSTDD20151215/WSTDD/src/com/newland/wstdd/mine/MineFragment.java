...
//		setUserVisibleHint(true);
	...
	private void setData(){
		...
		if (AppContext.getAppContext().getHeadImgUrl() != null && !"".equals(AppContext.getAppContext().getHeadImgUrl())) {
			ImageDownLoad.getDownLoadCircleImg(AppContext.getAppContext().getHeadImgUrl(), mMine_nickname_iv);
		}else {

			try {
				InputStream is = getResources().openRawResource(R.drawable.empty_photo);  
		...
		if(AppContext.getAppContext().getTags().size()>0&&AppContext.getAppContext().getTags().size()<=2){
			for (int i = 0,size = AppContext.getAppContext().getTags().size(); i < size; i++) {
				...
				stringBuffer.append(AppContext.getAppContext().getTags().get(0)+" ");
				stringBuffer.append(AppContext.getAppContext().getTags().get(1)+" ");
				stringBuffer.append("...");	
		}
