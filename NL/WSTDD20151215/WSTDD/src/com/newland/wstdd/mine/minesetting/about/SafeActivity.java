...
public class SafeActivity extends BaseFragmentActivity implements OnClickListener,OnPostListenerInterface {
		...
		try {
			new Thread(){
				public void run() {
					...
					safeReq.setPws(newpsw_edt.getText().toString());
	 ...
	 */
	private void judge() {
		if (StringUtil.isNotEmpty(oldpsw_edt.getText().toString())||
				StringUtil.isNotEmpty(newpsw_edt.getText().toString())||
				StringUtil.isNotEmpty(confirmnewpsw_edt.getText().toString())){
			if (oldpsw_edt.getText().toString().equals(appContext.getUserPwd())) {
