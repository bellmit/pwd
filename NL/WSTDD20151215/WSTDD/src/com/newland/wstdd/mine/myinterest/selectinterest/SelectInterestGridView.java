 ...
 *
 */
public class SelectInterestGridView extends GridView{
	/**
	 * DragGridView的item长按响应的时间， 默认是1000毫秒，也可以自行设置
	 */
	private long dragResponseMS = 500;
	private int startPosition=0;//第一次拽的时候的位置
	/**
	 * 是否可以拖拽，默认不可以
	 */
...
//	private Vibrator mVibrator;
	...
	private SelectInterestGridBaseAdapter mDragAdapter;
	...
	public SelectInterestGridView(Context context) {
//		this(context, null);
		super(context);
	}
	
	public SelectInterestGridView(Context context, AttributeSet attrs) {
//		this(context, attrs, 0);
		super(context,attrs);
	}

	public SelectInterestGridView(Context context, AttributeSet attrs, int defStyle) {
...
//			mVibrator.vibrate(50); //震动一下
	...
	public void setAdapter(ListAdapter adapter) {
		...
		if(adapter instanceof SelectInterestGridBaseAdapter){
			mDragAdapter = (SelectInterestGridBaseAdapter) adapter;
		}else{
			throw new IllegalStateException("the adapter must be implements SelectInterestGridBaseAdapter");
		}
	}
...
//    }
	...
	public boolean dispatchTouchEvent(MotionEvent ev) {
		...
		case MotionEvent.ACTION_UP:
			...
			int temp=pointToPosition(moveX1, moveY1);
...
//					onChanageListener.onChange(mDragPosition, tempPosition);
					onChanageListener.updatePosition(mDragPosition, tempPosition);
					return true;
				}
			} );
...
//			onChanageListener.onChange(mDragPosition, tempPosition);
	...
	public interface OnChanageListener{
		...
		public void updatePosition(int form, int to);
	}
