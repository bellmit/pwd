...
public class MineEditReceiptAddressActivity extends BaseFragmentActivity implements OnClickListener,OnPostListenerInterface{
	...
	private void judgeOk() {
		if (mMinereceiptaddress_receiptname_edt != null && StringUtil.isNotEmpty(mMinereceiptaddress_receiptname_edt.getText().toString())
				&& mMinereceiptaddress_phone_edt != null && StringUtil.isNotEmpty(mMinereceiptaddress_phone_edt.getText().toString())
				&& mMinereceiptaddress_area_content_tv != null && StringUtil.isNotEmpty(mMinereceiptaddress_area_content_tv.getText().toString())
				&& mMinereceiptaddress_address_edt != null && StringUtil.isNotEmpty(mMinereceiptaddress_address_edt.getText().toString())
				&& mMinereceiptaddress_postalcode_edt != null && StringUtil.isNotEmpty(mMinereceiptaddress_postalcode_edt.getText().toString())) {
	...
	public void onClick(View v) {
		...
		case R.id.minereceiptaddress_area_content_tv:
			((InputMethodManager)getSystemService(Context.INPUT_METHOD_SERVICE)).hideSoftInputFromWindow(MineEditReceiptAddressActivity.this.getCurrentFocus().getWindowToken(), InputMethodManager.HIDE_NOT_ALWAYS);  
			popWindow();
			break;
		default:
			break;
...
//				UiHelper.ShowOneToast(this, "14.	收货地址更新成功！！！"+mineAddAddressRes.getAddresses().get(0).getAddress());
